<template>
  <Dialog v-model:open="open">
    ...
    <DialogContent class="sm:max-w-md">
      <DialogHeader>
        <DialogTitle>{{ $t('transcription.add') }}</DialogTitle>
      </DialogHeader>
      <div class="space-y-4 py-4">
        ...
        <div class="space-y-2">
          <Label>{{ $t('transcription.providerType') }}</Label>
          <Select v-model:model-value="form.client_type">
            ...
            <SelectContent>
              <SelectGroup>
                <SelectItem
                  v-for="ct in transcriptionClientTypes"
                  :key="ct.value"
                  :value="ct.value"
                >
                  {{ ct.label }}
...
import { TRANSCRIPTION_CLIENT_TYPE_LIST } from '@/constants/client-types'
...
const transcriptionClientTypes = TRANSCRIPTION_CLIENT_TYPE_LIST
...
async function handleCreate() {
  ...
  try {
    ...
    open.value = false
    form.name = ''
    form.client_type = 'openai-transcription'
  } catch (error) {
    console.error('Failed to create transcription provider:', error)
