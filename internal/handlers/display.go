...
if ! display_socket_ready; then
  ...
  if ! display_socket_ready; then
    stop_xvnc
    cleanup_stale_display
    nohup "$XVNC" :99 -geometry 1280x800 -depth 24 -SecurityTypes None -rfbunixpath "$RFB_SOCKET" -rfbunixmode 0666 -rfbport 0 -BlacklistTimeout 0 -BlacklistThreshold 1000000 >/tmp/memoh-xvnc.log 2>&1 &
    wait_i=0
